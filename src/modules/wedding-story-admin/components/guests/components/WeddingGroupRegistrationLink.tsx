...
export const WeddingGroupRegistrationLink: React.FC<{
    weddingGroup: IWeddingGroup
    action: () => void
}> = ({ weddingGroup, action }) => {
    const weddingStoryClient = import.meta.env['VITE_OUR_STORY_URL']
    const url = `${weddingStoryClient}?registration=${weddingGroup.id}`
    ...
    return (
        <XMenuItem>
            <CopyToClipboard text={url}>
                ...
                >
                    <span className='flex w-[130px] justify-start capitalize'>Registration Link</span>
                </StyledButton>
            </CopyToClipboard>
        </XMenuItem>
    )
