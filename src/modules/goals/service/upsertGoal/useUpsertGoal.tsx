...
// import { isDashboard, isGoalFilterActive } from '@/services/guards'
...
//     useMutation({
//         mutationFn: ({ editGoal }: { editGoal: IGoalSchema }) => {
//             const { title, slogan, description, finished_at, id } = editGoal
//             const goalData: IGoalSchema = {
//                 id,
//                 owner_id: getUserId(),
//                 title,
//                 slogan,
...
//                     (oldData: IGoalSchema[] | { pages: { data: IGoalSchema[] }[] }) => {
...
//         onSettled: (data) => {
//             if (!data?.[0]) return
//             window.queryClient?.setQueryData(KEY_FetchGoalById(data[0].id), (oldData: IGoalSchema) => {
