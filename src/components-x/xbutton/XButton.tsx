...
export const XButton: React.FC<ButtonHTMLAttributes<HTMLButtonElement>> = (props) => {
    return (
        <button
            {...props}
            className={`relative cursor-pointer rounded-sm border border-transparent bg-button-bg
            ...
            disabled:cursor-not-allowed disabled:bg-gray-200 disabled:text-gray-400 ${props.className}`}
        >
            {props.children}
        </button>
    )
